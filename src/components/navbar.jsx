import { Menu, Mic, Search, UserCircle } from "lucide-react";
...
export function Navbar() {
    ...
    return (
        <div className="border-b border-gray-700 flex-shrink-0 h-[56px] w-full flex flex-row items-center justify-between p-2">
            ...
            <div className="flex flex-row items-center gap-4 h-full flex-grow justify-center mx-4">
                <SearchInput />
                <div className="hidden md:contents">
                    <MicSearch />
                </div>
            ...
            <div className="flex-shrink-0">
                {!user && <a href="/login" className="flex cursor-pointer select-none items-center p-2 px-4 hover:bg-blue-400/10 gap-2 border border-gray-500 rounded-full w-fit">
                    <UserCircle />
                    <span className="hidden md:contents">تسجيل الدخول</span>
                </a>}
                {!!user && <button onClick={() => {
                    localStorage.removeItem('user');
                    window.location.href = '/login';
                }} className="flex cursor-pointer select-none items-center p-2 px-4 hover:bg-blue-400/10 gap-2 border border-gray-500 rounded-full w-fit">
                    <img src={user.avatar} className="w-8 h-8 rounded-full" />
                    <span className="hidden md:contents">{user.username}</span>
                </button>}
            </div>
        </div>
    );
...
function SearchInput() {
    ...
    return (
        <div className="flex flex-row items-center gap-2 border border-gray-700 rounded-full h-full ps-4 flex-grow max-w-[600px]">
            {isFocused && <Search />}
            <input
                type="text"
                placeholder="البحث"
                className="focus:outline-none h-full bg-transparent w-full min-w-[100px]"
                onFocus={() => setIsFocused(true)}
                onBlur={() => setIsFocused(false)}
            />
            <button className="rounded-e-full rounded-s-none bg-[#272727] h-full px-4 flex-shrink-0" title="البحث">
                <Search size={24} />
            </button>
        </div>
    );
...
function MicSearch() {
    return <button className="rounded-full p-2 bg-[#272727] hover:bg-[#3D3D3D]">
        <Mic size={24} />
    </button>
}

export function SidebarButton() {
    const toggleIsOpen = useSidebarOpen(store => store.toggleIsOpen);
    return <button className="rounded-full p-2 hover:bg-gray-700" onClick={toggleIsOpen}>
        <Menu size={28} />
    </button>
...
export function YoutubeIcon() {
    return <a className="w-[80px] fill-white" href="/">
